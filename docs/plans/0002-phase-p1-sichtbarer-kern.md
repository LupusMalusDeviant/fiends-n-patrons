# Plan-0002: Phase P1 — Sichtbarer Kern (Toon-Renderer, 10k-Bullet-Stresstest, Sigil v1)
...
- **Toon-2.5D-Bühne:** gekippte Perspektivkamera (Neigung 60–75° konfigurierbar, Following eines Spieler-Proxys mit Look-Ahead, nur renderseitig), Mesh-Pass mit Tiefenpuffer, Toon-Shading mit 2–4 materialgesteuerten Bändern und Outlines (PRD-0003 FR-01/FR-03).
...
- **Alle P1-terminierten offenen Fragen entschieden:** OF-3.1, OF-3.2, OF-3.3, OF-4.1, OF-4.2, OF-16.2, OF-17.3, OF-18.1, OF-18.2 — dazu die PO-Entscheidung OF-16.1 (an M0).
- **Release:** Engine `v0.2.0` getaggt (additive API-Erweiterung, CHANGELOG), das Spiel pinnt den Tag (ADR-0009); jeder CI-Lauf beider Repos bis zum Ende überwacht.
...
|--------|-------------|-----------|
| PRD-0000 §5 | Toon-Renderer + Punktlichter + Kamera | WP2, WP3 |
...
| PRD-0002 Akzeptanz | Snapshot-Roundtrip bit-identisch; Beispiele je Subsystem | WP5.1; Beispiele (in CI nur gebaut): `toon_stage` (WP2.8), `lights_stage` (WP3.7), `sigil_curtain` (WP5.7), `profiler_dump` (WP6.3, Konsole), `collide_query` (WP6.5, Konsole), `pack_inspect` (WP8.3, Konsole), `stress_10k` (WP11.3) |
| PRD-0003 FR-01/02/03 | Toon-Pass + Outlines, ≥ 256 Lichter, gekippte Kamera | WP2.3–WP2.6, WP3.4 |
...
| PRD-0003 FR-14 | Deterministische Render-Testszenen (Toon, Lichter, Bullets) | WP2.8, WP3.6 |
...
| PRD-0017 FR-04 | Engine-Release mit Changelog, Spiel pinnt Tag | WP11.1 (Alpha-Tags), WP11.7 |
...
| OF-3.1 / 3.2 / 3.3 | Outlines, Schatten, Bullet-Darstellung | WP2.5, WP2.6, WP3.2 |
...
**Zweck:** Die stränge-übergreifenden Schnittstellen stehen als kompilierende Verträge mit Konformanztests, und die sprachprägenden Entscheidungen sind gefallen, bevor Sigil, Messung, Pipeline und Tooling parallel loslaufen.
...
### WP2: 2.5D-Bühne — Kamera, Mesh, Toon, Outlines

**Zweck:** Das erste Aha-Bild von P1: Die flache P0-Welt kippt in eine Toon-2.5D-Bühne mit Outlines — noch bevor Sigil existiert.
...
2. **WP2.2:** Render-Vertrag v1 additiv (vorhandene P0-Typen `Camera2D`, `SpriteInstance`, `RenderFrame` bleiben unverändert): `Camera25D` (Neigung, FOV, Ziel, Look-Ahead-Parameter, `screen_to_ground` als Strahl-Ebene-Schnitt), `MeshInstance`, `ToonMaterial` (Bänder, Farben, Outline-Stärke), `PointLight`, gerichtetes Key-Light + Ambient, Bullet-Kanal für den in WP1.2 definierten `BulletInstance`-Typ (hier nicht neu definiert); erweiterter `NullRenderer`; wgpu-Typen bleiben unsichtbar (Engine-ADR-0002). Review im Rahmen von WP1.7.
...
5. **WP2.5:** Toon-Shading mit materialgesteuerten Bändern; Spike OF-3.1 Inverted Hull gegen Screen-Space-Kanten aus Tiefe/Normalen an einer Szene mit 500 Meshes + 10k Sprites (Korrektheit offscreen über den Software-Adapter, Kosten relativ; absolut erst in der Messsitzung); Kriterium zusätzlich: Ebenen 4 und 6 bleiben unberührt. → Engine-ADR Outline-Technik, Gewinner umsetzen.
6. **WP2.6:** OF-3.2 Look-Dev: Blob-Shadows gegen keine Schatten anhand offscreen gerenderter Vergleichsbilder → PO-Entscheid, Eintrag in PRD-0003 und Stilbibel; bei Blob-Shadows ein einfacher Decal-Pass auf Ebene 1.
7. **WP2.7:** `docs/art/stilbibel.md` v0 im Spiel-Repo, als „vorläufig“ markiert (3 Referenzpaletten, Bandschwellen, Outline-Stärken, getrennte Palettenräume gegnerischer/eigener Projektile, Silhouette statt Farbe allein — die beiden letzten Regeln sind zusätzlich technisch erzwungen, siehe WP4.2 und WP3.5); Look-Dev-Timebox 1 Tag.
8. **WP2.8:** Render-Testszenen `toon_bands`, `outline`, `camera_tilt` mit `GRIMOIRE_GPU_ADAPTER=software`, Referenzen je Plattform mit Toleranzmetrik, Warnmodus. Schaufenster M1: Beispiel `toon_stage` (in CI nur gebaut) plus CI-Job, der offscreen eine PNG-Sequenz rendert und als GIF-Artefakt ablegt.

**Ergebnis:** Gekippte Toon-Bühne mit Outlines und Mauszielen über den Bodenstrahl; Adapter-Tabelle der Runner; Engine-ADR zu OF-3.1, Entscheid zu OF-3.2, Stilbibel v0; drei Render-Testszenen; M1-GIF als CI-Artefakt.
...
4. **WP3.4:** Clustered Forward+ nach ADR: Licht-Index-Storage-Buffer, Toon-Bänder verarbeiten Punktlichter, Lichtbudget in `RendererConfig` (Low 32 / High 256), Licht- und Cluster-Zähler in `RenderStats`.
...
6. **WP11.6:** Dokumentation („PRDs leben“, PRD-0000 §6.3): DoD-Checkliste gegen PRD-0000 §5 und diese Nachverfolgungstabelle; **PRD-0000** §4 Crate-Map (`grimoire_sigilc`, `grimoire_link`, `grimoire_bench`, `tools/`), §8 OF-2 als entschieden, §9 nächste Schritte; **PRD-0002** OF-2.1/OF-2.2 mit Verweis auf Engine-ADR-0004/-0003 geschlossen; **PRD-0003** Ebenendiagramm (Post-FX-Resolve vor Telegraphie und Bullets), OF-3.x; **PRD-0004** Pipeline-Text (Compiler = `grimoire_sigilc` nach ADR-0010), Budgetangabe vereinheitlicht (OP-1), OF-4.x; **PRD-0013** Achsenbelegung 2/3 und `i16`-Quantisierung der Zielrichtung; **PRD-0016** FR-04-Wortlaut an ADR-0010 angepasst, OF-16.1/16.2; **PRD-0017/0018** offene Fragen; Crate-Verträge P1 final, README-Status; Umsetzungsstand und Abweichungen in Plan-0002.
7. **WP11.7:** Release: Engine-CHANGELOG (git-cliff), Version `0.2.0` nach dem Release-Ablauf aus WP11.1, Tag `v0.2.0` mit überwachtem Release-Lauf; der Engine-Release bleibt nach `release.yml` „nur Quelltext“ (P-14). Lokal gebaute Windows-Werkzeug-Binaries (`sigilc`, `grimoire-link`, `grimoire-ac`, Editor) werden nach dem Build mit dem Signier-Skript des PO signiert (siehe OP-7), aber nicht am Release veröffentlicht. Spiel pinnt `v0.2.0` (ADR-0009), Spiel-CI auf 3 OS überwacht grün; P1-Abnahme mit dem PO.

**Ergebnis:** Integrierter sichtbarer Kern auf `main` beider Repos, Stresstest headless in CI und auf Referenz-Hardware gemessen, bestandenes Red-Team-Gate, aktualisierte PRDs, Engine `v0.2.0` getaggt und vom Spiel gepinnt — P1 Definition of Done.
...
| Von | Nach | Typ |
|-----|------|-----|
| M0 (P0 gepusht, Tag `v0.1.0`, Pin, ADR-0003/0004 akzeptiert) | P1-Start aller Stränge | intern, blocking |
| PO-Sammelsitzung A an M0 (u. a. P-1 = OF-16.1, P-3, P-4) | WP9, WP10 (P-1); WP6.5, WP11.2 (P-3); WP3.3, WP11.5 (P-4) | extern (PO), blocking |
...
| GitHub-Actions-Minuten (Bench-Spike, Unit-Identitäts-Gate auf 3 OS, dotnet auf 3 OS, GIF-Jobs) | WP6.1, WP4.4, WP9.1, Schaufenster-Jobs | extern (vor WP6.1 prüfen, OP-2) |
...
**Kritischer Pfad** (unter der Strang-Obergrenze, siehe Aktivierungsreihenfolge im Abschnitt Arbeitsorganisation): M0 → WP1 (10) → WP5 (14) → M2 → WP9 (8) → WP10.5 (≈ 1, WP10.1–10.4 laufen ab Ende WP4 vor) → WP11.4–WP11.7 (≈ 7) ≈ **40 fokussierte Tage Durchlauf** ohne Puffer; mit den Risikoreserven der Pfadpakete (WP1 +2, WP5 +3, WP9 +2, WP11 +3) ≈ 50 Tage plus anteiliger Puffer. Nahezu kritisch: M0 → WP1 → WP4 (12) → WP7 (Start wegen Strang-Obergrenze erst nach WP6, ≈ Tag 20) → WP11 (≈ 39 Tage).
...
**Rückfall bei Engpass (E20):** OF-16.1 wird genau einmal entschieden, an M0: entweder ein Ort für die C#-Suite, oder ausdrücklich „C#-Seite (WP9, WP10) nach P2“ — dann tragen WP8 und `grimoire-link` den DoD-Nachweis allein. Überschreitet der Tooling-Anteil (WP9–WP10) seine Schätzung inklusive Reserve deutlich, wandert der Sigil-Editor-MVP nach PO-Entscheid in P2; Asset-Compiler-Gate, Live-Link-Basis und `grimoire-link` bleiben P1.
...
| Sigil-Sprache | WP4 | WP1 | übernimmt Slot 1 nach WP1 (≈ Tag 10) |
| Sigil-Laufzeit | WP5 | WP1 | übernimmt Slot 2 nach WP2 (≈ Tag 10) |
| Pipeline-Rust | WP8 | WP1 | übernimmt Slot 4 nach WP3 (≈ Tag 16) |
| Absicherung | WP7 | WP1, Harness nach WP5.1 | übernimmt Slot 3 nach WP6 (≈ Tag 20) |
| Tooling | WP10 | WP4.3, WP5.6; WP10.5 nach WP9.1 | übernimmt Slot 1 nach WP4 (≈ Tag 22) |
| Pipeline-C# | WP9 | M2 + P-1 entschieden | übernimmt Slot 2 nach WP5 = M2 (≈ Tag 24) |
| Integration | WP11 | rollierend je Meilenstein; WP11.2 ff. wie in Abhängigkeiten | WP11.1 durch den jeweils mergenden Strang; WP11.2 ff. im ersten freien Slot nach WP7/WP8 (≈ Tag 29), Abschluss zuletzt |
...
- **Pins:** Das Spiel pinnt während P1 `v0.2.0-alpha.N`; der lokale `[patch]` in `<Arbeitsordner>\.cargo\config.toml` bleibt in Agentenläufen auskommentiert (ADR-0009).
...
| R11 | WP6: Kollisionsbudget ≤ 1,5 ms bei 10k Bullets + 100 Gegnern + Graze-Query single-threaded (Engine-ADR-0003) nicht haltbar | niedrig | mittel | Headless-Bench in der ersten Strangwoche von WP6.5; Zellgrößen-Tuning, SoA; Migrationspfad zur Parallelität mit fester Reduktionsreihenfolge laut Engine-ADR-0003 als dokumentierter Plan B |
...
| R15 | WP2: Look-Dev mit prozeduralen Primitiven überzeugt nicht, der Toon-Look wird zum Zeitfresser; Sog in Mesh-Pack oder glTF-Import | mittel | mittel | Stilbibel v0 als „vorläufig“ vor der Shader-Arbeit, Timebox 1 Tag; PO-Look-Review nur anhand der Offscreen-Bilder; Mesh-Chunks und glTF ausdrücklich P3 |
...
| R17 | WP11/übergreifend: Parallele Stränge und der gleichzeitig laufende Engine-Workflow kollidieren an Fassade, `crate-vertraege.md`, Workspace-Manifest und ADR-Nummern | mittel | mittel | Feste Merge-Reihenfolge je Meilenstein; Rebase vor jedem Merge; Vertragsänderungen nur als Vertrags-PR; ADR-Nummern beim Merge; Alpha-Tags statt `[patch]` |
...
| R21 | Extern: GitHub-Actions-Minuten des privaten Plans reichen nicht für Bench-Spike, Unit-Identitäts-Gate, dotnet auf 3 OS und GIF-Jobs | mittel | mittel | Kontingent vor WP6.1 prüfen (OP-2); Bench-Spike zuerst nur Linux; 3-OS-Jobs mit Pfadfiltern, GIF- und Selbsttest-Jobs nur nightly bzw. an Meilensteinen; bei Engpass Matrix reduzieren (Windows/macOS nightly statt pro Push) und dem PO melden |
| R22 | Organisatorisch: Stau offener PO-Entscheidungen (P-1 bis P-14) blockiert die Stränge am Ende von WP1 | hoch | mittel | Zwei gebündelte Sammelsitzungen (A an M0, B am Ende von WP1) mit vorbereiteten Empfehlungen statt Einzelanfragen; Stränge laufen bis dahin gegen die Empfehlung, ohne zu mergen; +1 Tag Reserve auf WP1 |
...
- **M0 „Startklar“ (Eintritts-Gate, kein P1-Aufwand):**
  ...
  - Goldener Determinismus-Hash stimmt auf Windows, Linux und macOS überein; Engine-ADR-0003/-0004 daraufhin akzeptiert; P0-Review-Befunde gemergt.
...
- **M1 „Die Welt kippt“ (WP1 + WP2 fertig; WP3.1–3.3 und WP6.1–6.2 fertig):**
  - Gekippte Toon-Bühne mit Outlines, Spieler-Proxy und Mauszielen als Offscreen-GIF im CI.
  ...
  - Go/No-Go auf Runner-Werten dokumentiert; Messsitzung 1 mit dem PO terminiert (durchgeführt spätestens an M2); `v0.2.0-alpha.1` nach Release-Ablauf.
- **M2 „Der erste Vorhang“ (WP3 + WP4 + WP5 + WP6.3–6.4 fertig):**
  ...
  - Render-Testszenen nach Blockier-Regel; Befund OF-18.2 in PRD-0018; `v0.2.0-alpha.2`.
- **M3 „Live-Magie“ (WP6 + WP7 + WP8 fertig; WP9 fertig oder laut M0-Beschluss nach P2):**
  ...
  - ADR Debug-Link und OF-18.1-Eintrag; `v0.2.0-alpha.3`.
- **M4 „Sichtbarer Kern“ (WP10 + WP11 fertig) = P1 Definition of Done:**
  ...
  - Alle P1-Fragen entschieden, PRDs nach WP11.6 aktualisiert; Engine `v0.2.0` getaggt, Spiel pinnt `v0.2.0`, alle CI-Läufe beider Repos überwacht grün.
...
- OF-3.1, OF-3.2, OF-3.3, OF-4.1, OF-4.2, OF-16.2, OF-17.3, OF-18.1 und OF-18.2 sind per ADR, Vertragseintrag oder PRD-Aktualisierung entschieden; OF-16.1 ist vom PO an M0 entschieden; `docs/art/stilbibel.md` v0 existiert; die in WP11.6 genannten PRD-Stellen sind nachgezogen.
...
- Engine `v0.2.0` ist mit generiertem CHANGELOG getaggt, jeder Alpha-Tag ist über den Release-Ablauf entstanden, die P0-APIs sind unverändert (additive Erweiterung), `wgpu`-Typen erscheinen nicht in der Fassaden-API, das Standalone-Gate ist grün; das Spiel baut mit `--locked` gegen den Tag; jeder P1-Push wurde bis zum Ende überwacht, jeder rote Lauf hat eine benannte Ursache; die Standard-Push-CI beider Repos bleibt < 15 Min pro Plattform.

## Zeitschätzung (Gesamt)

- **Summe Arbeitspakete:** 115 fokussierte Arbeitstage (WP1: 10, WP2: 10, WP3: 10, WP4: 12, WP5: 14, WP6: 10, WP7: 9, WP8: 13, WP9: 8, WP10: 9, WP11: 10)
- **Puffer (25 %):** 29 Tage
- **Reserve für hohe Risiken:** 16 Tage. Einheitliche Regel: Jedes Risiko mit mindestens einer Bewertung „hoch“ und keiner Bewertung „niedrig“ bekommt Reserve auf sein Hauptpaket — +2 Tage bei hoch × hoch oder bei einem Budget-Risiko mit hohem Impact (R1, R2, R3), sonst +1 Tag. Verteilung: WP1 +2 (R5, R22), WP3 +3 (R2, R6), WP5 +3 (R3, R4), WP6 +1 (R10), WP8 +1 (R7), WP9 +2 (R1), WP10 +1 (R20), WP11 +3 (R9, R18, R19). R16 (niedrig × hoch) und alle mittel × mittel-Risiken sind durch den Puffer gedeckt.
- **Gesamt:** ~160 fokussierte Arbeitstage — wegen der Parallelität der Stränge ist die Durchlaufzeit deutlich kürzer als die Summe (kritischer Pfad ≈ 40 Tage, mit Pfadreserven ≈ 50 Tage, plus anteiliger Puffer); im Hobby-Rhythmus bewusst ohne Kalender-Zusage (E19).
...
- **P-1 — OF-16.1, Repo-Ort der C#-Tooling-Suite** (Empfehlung `grimoire/tools/`, weil Protokoll und Formate engine-versioniert sind; Folge: .NET-Job in der Engine-CI mit Pfadfiltern). **Sammelsitzung A, M0 — einzige Entscheidungsstelle;** Alternative ist der ausdrückliche Beschluss „C#-Seite nach P2“. Blockiert WP9 und WP10.
- **P-2 — Projekt-ADR-0010 „Sigil-Compiler-Hoheit“ bestätigen:** Rust (`grimoire_sigilc`/`sigilc`) als einzige Parser-/Compiler-Implementierung, die Laufzeit lädt nur Binär-Units, der C#-Asset-Compiler orchestriert per CLI/JSON, die Editor-Vorschau nutzt den Rust-Interpreter statt einer C#-Canvas-Sim. Präzisiert E07/ADR-0007 und weicht vom Wortlaut PRD-0016 FR-04 ab (Empfehlung: ja; Folgen einer Ablehnung siehe R20). **Sammelsitzung B.**
- **P-3 — `grimoire_collide` v0 in P1:** Grid, Kreis/Kapsel, Layer-Masken, Graze-Ring-Query als Budget-Nachweis, Gameplay-Anbindung in P2 (Empfehlung: ja, weil die P1-Abnahme in PRD-0002 das Kollisionsbudget verlangt). Bei Nein wird der Kollisionsanteil des Stresstests ausdrücklich nach P2 verschoben und die P1-Abnahme als Teilnachweis dokumentiert. **Sammelsitzung A.**
...
- **P-5 — Umfang der Referenz-Patterns:** ≥ 12 in P1 mit voller Abdeckung, Rest bis 20 in P2 (Empfehlung) oder alle 20 in P1. **Sammelsitzung A.**
- **P-6 — Umfang des Sigil-Editor-MVP und Überzugsregel:** Text + Parameter-Panel + Vorschau + Push in P1, Knoten-Komponieren nach E20 in P2; Rückfall: Editor-MVP nach P2, wenn der Tooling-Anteil deutlich überzieht, Asset-Compiler-Gate und Live-Link bleiben P1 (Empfehlung: ja). **Sammelsitzung A.**
- **P-7 — Pin- und Versionspolitik in P1:** Vorab-Tags `v0.2.0-alpha.N` je Meilenstein über den Release-Ablauf (WP11.1), Spiel pinnt Alpha-Tags; `v0.2.0` als Abschluss mit rein additiver API (Empfehlung: ja). **Sammelsitzung A.**
- **P-8 — Replay-Format v2** mit Engine-Version, Build-Hash, Content-Manifest-Hash und Anwendungs-Metadaten schon in P1 (Voraussetzung dafür, dass Golden Master Content-Änderungen erkennen; Empfehlung: ja). **Sammelsitzung A.**
- **P-9 — Ablageort der Format-Dokumentation:** `grimoire/docs/formats/` (engine-versioniert) mit Verweis aus dem Spiel-Repo, abweichend vom Wortlaut „docs/formats/“ in PRD-0016 FR-10 (Empfehlung: ja). **Sammelsitzung A.**
- **P-10 — Abnahme der Spike-ADRs:** Sigil-Syntax OF-4.1 und Schema-Codegen OF-16.2 (Sammelsitzung B), Outline-Technik OF-3.1, Bullet-Darstellung OF-3.3, Licht-Culling und Go/No-Go (M1).
- **P-11 — OF-3.2 Schatten und Stilbibel v0:** Blob-Shadows oder keine (Empfehlung Blob-Shadows) sowie Paletten, Bänder, Outline-Stärken und Bullet-Palettenräume. **An M1.**
- **P-12 — Benchmark-Trendablage und Rückfall:** Datenzweig, Actions-Artefakte oder GitHub Pages im privaten Repo; falls der OF-17.3-Spike keine rauscharme Metrik auf gehosteten Runnern findet: eigener Server ohne GPU (nie der Entwicklungsrechner) oder größerer kostenpflichtiger Runner. **Mit dem OF-17.3-ADR, spätestens M2.**
- **P-13 — .NET-SDK- und Avalonia-Version** (Vorschlag: aktuelle LTS), weil sie die CI-Matrix betrifft. **Mit P-1.**
- **P-14 — Werkzeug-Binaries am Engine-Release:** `release.yml` und `CONTRIBUTING.md` legen „nur Quelltext“ fest, das Zertifikat liegt nur im lokalen Speicher. (Empfehlung: in P1 bleibt der Release quelltext-only, Werkzeuge werden aus dem Tag gebaut; Veröffentlichung signierter Werkzeug-Binaries zusammen mit der Distribution in P3 — dann per Engine-ADR, das `release.yml` erweitert oder einen lokalen Build mit dem Signier-Skript des PO und `gh release upload` festschreibt.) **Sammelsitzung A.**
...
- **OP-2:** GitHub-Actions-Minuten des privaten Plans reichen für Bench-Spike, Unit-Identitäts-Gate auf 3 OS, dotnet-Jobs auf 3 OS und Offscreen-GIF-Jobs? Vor WP6.1 prüfen (R21).
...
- **OP-6:** Die Spiel-CI checkt das Engine-Repo am gepinnten Tag über den vorhandenen Deploy-Key aus (ADR-0009 beschreibt ihn bisher nur für den Cargo-Git-Fetch) — Nutzung durch `actions/checkout` im ADR-0009-Umsetzungsteil nachtragen; Cache-Schlüssel und Laufzeit in WP9.3 verifizieren.
- **OP-7:** Signierpflicht für Windows-Binaries — **Konflikt zur PO-Entscheidung:** Die Signierregel des PO verlangt eine Signatur nach **jedem** Build einer Windows-Binary; PRD-0000 §6.6 spricht nur von Release-Builds. Bis der PO entscheidet, gilt die strengere Signierregel des PO (jede lokal gebaute Windows-Binary wird signiert); CI-Builds auf Runnern bleiben unsigniert, weil das Zertifikat nur lokal liegt, und werden nicht weitergegeben. Je nach Entscheid wird PRD-0000 §6.6 in WP11.6 angeglichen oder die Signierregel des PO präzisiert.
